...
def get_county(lat, lon, conn: Connection):
    cur = conn.cursor()
...
def find_jurisdiction(lat, lon, conn: Connection):
    '''
...
def simple_lookup(conn: Connection, address = None):
    '''
...
def get_best_fit_codes(
    ...
    logger.debug("/lookup/request :", address)
    lat, lon = None
    if address :
        lat, lon = geocode_address(address)
    else :
        logger.warning("/lookup : Address field left blank")
    
    if lat is None or lon is None:
        logger.warning("/lookup : Failed to find geocode address")
